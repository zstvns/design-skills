...
Name the depth (see `design-principles` → Scoping the Depth):

- **Revolution** — building or rebuilding from the ground up. Full strategy: vision, muses, distinct concepts, latitude.
- **Evolution** — keeping equity (often logo + core palette) while refreshing expression. Lighter; anchored on what stays.
- **Optimization** — the brand is fine; execution is the problem. You may not need this skill — go to the medium and fix the execution.

Match the effort to the depth. Revolutionizing a brand that only needed optimization throws away hard-won recognition.
...
What does the category look like right now — the colors, type, imagery everyone uses? Not to copy; to know what "distinct" requires. You can't be distinct if you don't know what everyone else is doing.
...
Distill everything into the few feelings the brand must evoke. Three is a good number. These become the rubric every later design is judged against.
...
- Present them as real intended assets (stylescapes), not safe mood boards — expressive and free, not yet constrained by buildability
