...
- Each is a **genuinely different answer to the same Emotional Target** — different muses, different color story, different type pairing, different attitude. Not three tints of one idea.
...
- Build each far enough to be judged: palette with roles, type pairing in real use, the motif, one hero-scale composition, treatment of photography/illustration/icons.

## Composing a stylescape

- One wide board per concept (traditionally ~1440px tall, several thousand wide), reading left → right from *foundation* (color, type) through *expression* (motif, imagery) to *application* (hero fragment, collateral hint).
- Use **real copy** from the brand — lorem hides whether the type actually serves the message.
...
- Run the self-audit (foundational categories vs. the competitor set + AI-slop check).
