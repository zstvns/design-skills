...
- **The type pairing is a *decision*, not an A/B.** A stylescape commits: name the **headline** face and the **supporting** face and show them doing their jobs, with **large type samples**. Do **not** put a typeface switcher / tabs on the board — deciding is the work. And **use real, characterful faces** (embed them if the canvas blocks webfonts) — a generic system serif (Georgia/Times as-is) reads amateur and will sink the whole board; pick a face with a point of view (an inscriptional Cinzel for imperial, a machined Chakra Petch for a console).
