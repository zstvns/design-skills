...
- **Break the template — each concept gets its own *composition*, not the same grid recolored.** The single biggest tell of shallow work is three boards with identical layout, identical tile order, identical icon style, identical interaction — just swapped colors and fonts. Let the muse drive a *different arrangement* each time: the console might be a control-panel grid; the orchestral a flowing horizontal score; the imperial a centered, monumental, symmetrical stele. If the three share a structure, they become *harder* to compare, not easier, and every one reads contrived. When the layout feels same-y, the muse hasn't taken over yet.
- **No decorative afterthoughts.** Every element earns its place and, on a live canvas, does something. A static ornament dropped in "for flavor" that doesn't play into the motif and can't be interacted with (a stray seal, a lone badge) reads as filler and cheapens the whole board — cut it or make it load-bearing.
...
- **Make the tool loop frictionless.** When assets come from an external generator, put a **one-click "copy prompt"** on each GPT-Image slot and an **"upload result"** control that drops the returned image straight into the slot — so the founder can round-trip prompt→ChatGPT→board without fighting the canvas.
