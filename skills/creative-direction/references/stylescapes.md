...
- **Show a full type specimen of *every* face used** — not one word. Headline and supporting must show real contrast (weight, structure, register); if they read similar, the pairing is wrong.
...
- **Reserve the accent; interaction states use neutrals.** Don't paint every hover/active/selected state in the primary accent — that over-uses the one color that's supposed to mean "action" and flattens the hierarchy. Let neutrals and supporting colors carry interaction feedback; keep the accent for what actually matters.
