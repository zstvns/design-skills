...
- Typography is chosen to **suit the direction**, not literally pulled from the muse (muse-derived type is rare — and a *literal* muse-face, an all-caps Roman inscriptional for an imperial muse, reads as costume and cliché). Evoke the muse with a strong, clean pairing; the abstraction lives in motif, color, texture, and composition first.
...
- **Populate real imagery from the start — don't ship a board of empty prompt-slots.** Pull free stock (Unsplash/Pexels) *yourself* to establish the photographic tone immediately, so the direction is *felt* without a founder step; assume nothing paid. Reserve the copy-prompt/upload loop for the muse-specific shots free stock can't cover. The goal is fewer steps for the founder, not a to-do list of prompts.
